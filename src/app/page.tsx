    ...
    <div className="flex min-h-screen flex-col items-center bg-white">
      <div className="container mx-auto flex max-w-5xl flex-col items-center px-4 pt-12 md:pt-16">
        <div className="mb-6 flex items-center justify-center gap-2">
          ...
          <h1 className="text-center text-3xl font-bold text-slate-800">Try Business Class for 30 days</h1>
        </div>

        <p className="mb-10 max-w-3xl text-center text-slate-600">
          Trello Business Class is built for teams who get things done. For 30 days, your team can use all of Trello's
          Business Class features for free.
        </p>
        ...
        <div className="mb-8 grid w-full grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div
            className={cn(
              "flex cursor-pointer flex-col rounded-lg border-2 p-6 transition-all",
      ...
      <div className="w-full bg-gray-100 py-4 text-center">
        <button className="text-slate-600 hover:text-slate-800 hover:underline">Start without Business Class</button>
      </div>
    </div>
  )
}
...
function CompanyLogo({ name }: { name: string }) {
  return <div className="h-6 w-20 rounded bg-gray-200 text-center text-xs text-gray-500 leading-6">{name}</div>
}
